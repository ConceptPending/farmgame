...
describe("First Harvest (PR Q tuning)", () => {
  it("Easy: greedy baseline wins ≥80% — tutorial completion is near-guaranteed", () => {
    // Easy: startingMoney 1500, expenseMultiplier 0.7, target 2000 (=2500*0.8).
    // Target raised $1600 → $2000 in PR V to keep curve after drought tuning.
    const batch = runBatch({ startingMoney: 1500, expenseMultiplier: 0.7, target: 2000 });
  ...
  it("Normal: greedy baseline wins 20-50% — humans target 50-60%", () => {
    // Normal: startingMoney 750, expenseMultiplier 1.0, target 2500 (=2500*1.0).
    const batch = runBatch({ startingMoney: 750, expenseMultiplier: 1.0, target: 2500 });
  ...
  it("Hard: greedy baseline wins 5-30% — punishing but completable", () => {
    // Hard: startingMoney 800, expenseMultiplier 1.3, target 3125 (=2500*1.25).
    const batch = runBatch({ startingMoney: 800, expenseMultiplier: 1.3, target: 3125 });
